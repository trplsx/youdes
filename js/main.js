...
document.addEventListener('DOMContentLoaded', () => {
    ...
    tabButtons.forEach(button => {
        button.addEventListener('click', () => {
            // Удаляем активный класс со всех кнопок и снимаем secondary стиль
            tabButtons.forEach(btn => {
                btn.classList.remove('active');
                // Для всех кнопок добавляем secondary стиль
                btn.classList.add('gallery__nav-btn--secondary');
            });

            // Убираем active у всех информационных блоков
            heroInfo.classList.remove('active');
            heroInfoModal.classList.remove('active');

            // Добавляем активный класс нажатой кнопке и убираем secondary
            button.classList.add('active');
            button.classList.remove('gallery__nav-btn--secondary');

            // Получаем имя таба и показываем соответствующий блок
            const tabName = button.getAttribute('data-tab');
